...
import { reorderDimensions } from './reorder-dimensions.mjs';

const SURVEY = 'ROAD_GO_NA_RL3G';
const FIELD = { survey: 'survey', dimensions: 'dimensions', obsHR: 'obsHR' };
const BATCH_SIZE = 5000;
const PAGE_SIZE = 5000;
const PROGRESS_EVERY = 50000;

const CFG = { pins: { 0: 'rl3g_goods' }, last: ['rl3g_year'] };
const CAT = { rl3g_goods: 'nst07', rl3g_year: 'time_period' };
...
function selfTest() {
  ...
  const dims = ['Nordburgenland', 'Products of agriculture', 'Thousand tonnes', 'Annual', '2010'];
  const obsHR = {
    geo: 'Nordburgenland',
    nst07: 'Products of agriculture',
  ...
  A(r.dimensions[0] === 'Products of agriculture', '[0] = tipo-merce (nst07)');
  A(r.dimensions[r.dimensions.length - 1] === '2010', 'ultima = anno');
  A([...r.dimensions].sort().join() === [...dims].sort().join(), 'nessuna label persa');

  const year = r.dimensions.find(d => /^\d{4}$/.test(d)) || '';
  const mode = r.dimensions.find(d => !['Annual', year].includes(d)) || '';
  A(mode === 'Products of agriculture', `mode-find del frontend prende la merce, non la geo (mode="${mode}")`);
